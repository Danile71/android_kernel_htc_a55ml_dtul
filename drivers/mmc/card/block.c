			     ...
			     struct mmc_async_req *areq)
{
	...
	if (gen_err) {
		pr_warn("%s: retrying write for general error\n",
				req->rq_disk->disk_name);
		if (card->host->card_err_count++ > MMC_MAX_CARD_ERR_COUNT) {
			pr_err("%s: card error count overflow\n", req->rq_disk->disk_name);
			mmc_card_set_removed(card);
			card->host->card_err_count = 0;
		}
		return MMC_BLK_RETRY;
	}
...
#define UP_LIMITS_4BYTE		4294967295UL	
#define ID_CNT 10
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *rqc)
{
	...
	if (rqc)
		reqs = mmc_blk_prep_packed_list(mq, rqc);
			...
			if(t_period >= (unsigned long long )PRT_TIME_PERIOD)
			{
				...
				{
					do_div(mmcqd_t_usage_wr[idx], 1000000);	
					if(mmcqd_t_usage_wr[idx])	
					{
						perf_meter = (mmcqd_rq_size_wr[idx])/((unsigned int)mmcqd_t_usage_wr[idx]); 
				...
				{
					do_div(mmcqd_t_usage_rd[idx], 1000000);	
					if(mmcqd_t_usage_rd[idx])	
					{
						perf_meter = (mmcqd_rq_size_rd[idx])/((unsigned int)mmcqd_t_usage_rd[idx]); 
	...
	do {
		...
		if (mmc_card_sd(card) && mmc_card_removed(card)) {
			...
			pr_warning("%s: stop retry, prepare to remove SD card\n",
						mmc_hostname(card->host));
			goto cmd_abort;
		}
