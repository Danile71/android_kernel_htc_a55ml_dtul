...
kal_int32 gFG_capacity_by_hwocv_init = 0;
...
kal_int32 gFG_hwocv_init = 0;
kal_int32 gFG_swocv_init = 0;
...
kal_int32 gFG_BATT_CAPACITY_aging_offset = 0;

kal_int32 gfg_percent_check_point = 50;
kal_int32 volt_mode_update_timer = 0;
kal_int32 volt_mode_update_time_out = 6;	
...
void fgauge_get_profile_id(void)
{
	...
	id_volt = PMIC_IMM_GetOneChannelValue(BATTERY_ID_CHANNEL_NUM, 1, 0);
	id_volt = (id_volt*VOLTAGE_FULL_RANGE)/ADC_PRECISE_CH7;
...
{
#if defined(CONFIG_MTK_BATT_TEMP_NOT_READY)
	return 1;
#elif defined(MTK_MULTI_BAT_PROFILE_SUPPORT)
	
	
	if ((g_fg_battery_id + 1) == TOTAL_BATTERY_NUMBER) {
		bm_print(BM_LOG_CRTI, "Battery id error (%d)\n", g_fg_battery_id);
		return FG_ERROR_BATTERY;
	} else
		return (g_fg_battery_id + 1);
#else
	return 1;
#endif
}
...
kal_int32 fgauge_get_Q_max(kal_int16 temperature)
{
    ...
    if( temperature < gTemperature_map_tbl[0] ){
        ret_Q_max = g_Q_MAX[g_fg_battery_id][0];

    }else if( temperature > gTemperature_map_tbl[TOTAL_TEMP_MAP_NUM - 1] ){
        ret_Q_max = g_Q_MAX[g_fg_battery_id][TOTAL_TEMP_MAP_NUM - 1];

    }else{
        for( i = 1; i < TOTAL_TEMP_MAP_NUM; i++){
            if( temperature <= gTemperature_map_tbl[i] ) {

                if( temperature == gTemperature_map_tbl[i-1] ){
                    ret_Q_max = g_Q_MAX[g_fg_battery_id][i-1];

                }else if( temperature == gTemperature_map_tbl[i] ){
                    ret_Q_max = g_Q_MAX[g_fg_battery_id][i];

                }else{
                    low_temperature = gTemperature_map_tbl[i-1];
                    low_Q_max = g_Q_MAX[g_fg_battery_id][i-1];
                    high_temperature = gTemperature_map_tbl[i];
                    high_Q_max = g_Q_MAX[g_fg_battery_id][i];
                }
                break;
            }
        }
...
kal_int32 fgauge_get_Q_max_high_current(kal_int16 temperature)
{
    ...
    if( temperature < gTemperature_map_tbl[0] ){
        ret_Q_max = g_Q_400M_MAX[g_fg_battery_id][0];

    }else if( temperature > gTemperature_map_tbl[TOTAL_TEMP_MAP_NUM - 1] ){
        ret_Q_max = g_Q_400M_MAX[g_fg_battery_id][TOTAL_TEMP_MAP_NUM - 1];
        ...
        for( i = 1; i < TOTAL_TEMP_MAP_NUM; i++){
            if( temperature <= gTemperature_map_tbl[i] ) {
                ...
                if( temperature == gTemperature_map_tbl[i-1] ){
                    ret_Q_max = g_Q_400M_MAX[g_fg_battery_id][i-1];

                }else if( temperature == gTemperature_map_tbl[i] ){
                    ret_Q_max = g_Q_400M_MAX[g_fg_battery_id][i];

                }else{
                    low_temperature = gTemperature_map_tbl[i-1];
                    low_Q_max = g_Q_400M_MAX[g_fg_battery_id][i-1];
                    high_temperature = gTemperature_map_tbl[i];
                    high_Q_max = g_Q_400M_MAX[g_fg_battery_id][i];
                }
                break;
            }
        }
...
kal_int32 fgauge_get_Q_max(kal_int16 temperature)
{
    ...
    if( temperature < gTemperature_map_tbl[0] ){
        ret_Q_max = g_Q_MAX_S[0];

    }else if( temperature > gTemperature_map_tbl[TOTAL_TEMP_MAP_NUM - 1] ){
        ret_Q_max = g_Q_MAX_S[TOTAL_TEMP_MAP_NUM - 1];
        ...
        for( i = 1; i < TOTAL_TEMP_MAP_NUM; i++){
            if( temperature <= gTemperature_map_tbl[i] ) {
                ...
                if( temperature == gTemperature_map_tbl[i-1] ){
                    ret_Q_max = g_Q_MAX_S[i-1];

                }else if( temperature == gTemperature_map_tbl[i] ){
                    ret_Q_max = g_Q_MAX_S[i];

                }else{
                    low_temperature = gTemperature_map_tbl[i-1];
                    low_Q_max = g_Q_MAX_S[i-1];
                    high_temperature = gTemperature_map_tbl[i];
                    high_Q_max = g_Q_MAX_S[i];
                }
                break;
            }
        }
...
kal_int32 fgauge_get_Q_max_high_current(kal_int16 temperature)
{
    ...
    if( temperature < gTemperature_map_tbl[0] ){
        ret_Q_max = g_Q_400M_MAX_S[0];

    }else if( temperature > gTemperature_map_tbl[TOTAL_TEMP_MAP_NUM - 1] ){
        ret_Q_max = g_Q_400M_MAX_S[TOTAL_TEMP_MAP_NUM - 1];
        ...
        for( i = 1; i < TOTAL_TEMP_MAP_NUM; i++){
            if( temperature <= gTemperature_map_tbl[i] ) {
                ...
                if( temperature == gTemperature_map_tbl[i-1] ){
                    ret_Q_max = g_Q_400M_MAX_S[i-1];

                }else if( temperature == gTemperature_map_tbl[i] ){
                    ret_Q_max = g_Q_400M_MAX_S[i];

                }else{
                    low_temperature = gTemperature_map_tbl[i-1];
                    low_Q_max = g_Q_400M_MAX_S[i-1];
                    high_temperature = gTemperature_map_tbl[i];
                    high_Q_max = g_Q_400M_MAX_S[i];
                }
                break;
            }
        }
...
int BattVoltToTemp(int dwVolt)
{
	...
	int sBaTTMP = -100;
     ...
     if (dwVolt > (RBAT_PULL_UP_VOLT - 100))
         return LOWEST_TEMP;
...
int force_get_tbat(kal_bool update)
{
...
#ifdef A55ML_BATT
	if(!of_machine_hwid()){
		bm_print(BM_LOG_CRTI, "[force_get_tbat] fixed TBAT=25 t for XA board.\n");
		return 25;
	}
#endif
	...
	if ((bat_temperature_val > 65) && (BMT_status.test_power_monitor || BMT_status.keep_charger_on))
		return 65;
...
{
	return sizeof(battery_profile[0][0]) / sizeof(BATTERY_PROFILE_STRUC);
}

int fgauge_get_saddles_r_table(void)
{
	return sizeof(r_profile[0][0]) / sizeof(R_PROFILE_STRUC);
}
...
BATTERY_PROFILE_STRUC_P fgauge_get_profile(kal_int32 temperature)
{
    ...
    if( temperature == TEMPERATURE_T ){
        if( unlikely(battery_profile_temperature == NULL))
            battery_profile_temperature = (BATTERY_PROFILE_STRUC_P) kmalloc(PROFILE_SIZE * sizeof(BATTERY_PROFILE_STRUC), GFP_KERNEL);

        return battery_profile_temperature;
    }

    for( i = 0; i < TOTAL_TEMP_MAP_NUM; i++){
        if( temperature == gTemperature_map_tbl[i])
            return &battery_profile[g_fg_battery_id][i][0];
    }
...
R_PROFILE_STRUC_P fgauge_get_profile_r_table(kal_int32 temperature)
{
    ...
    if( temperature == TEMPERATURE_T ){
        if( unlikely(r_profile_temperature == NULL))
            r_profile_temperature = (R_PROFILE_STRUC_P) kmalloc(fgauge_get_saddles_r_table() * sizeof(R_PROFILE_STRUC), GFP_KERNEL);

        return r_profile_temperature;
    }

    for( i = 0; i < TOTAL_TEMP_MAP_NUM; i++){
        if( temperature == gTemperature_map_tbl[i])
            return &r_profile[g_fg_battery_id][i][0];
    }
	...
	for (i = 0; i < saddles - 1; i++) {
		if ((voltage <= (profile_p + i)->voltage)
			...
			ret_percent =
			    ...
			    DIV_ROUND_CLOSEST(((((profile_p + i)->voltage) -
...
kal_int32 fgauge_read_v_by_capacity(int bat_capacity)
{
	...
	for (i = 0; i < saddles - 1; i++) {
		if ((bat_capacity >= (profile_p + i)->percentage)
			...
			ret_volt =
			    (profile_p + i)->voltage -
			    DIV_ROUND_CLOSEST(((bat_capacity -
...
{
	BATTERY_PROFILE_STRUC_P temp_profile_p, profile_p[TOTAL_TEMP_MAP_NUM];
	int i, j, saddles, profile_index;
	kal_int32 low_p = 0, high_p = 0, now_p = 0, low_vol = 0, high_vol = 0;
	
    for( i = 0; i < TOTAL_TEMP_MAP_NUM; i++)
        profile_p[i] = fgauge_get_profile(gTemperature_map_tbl[i]);

	saddles = fgauge_get_saddles();
	temp_profile_p = (BATTERY_PROFILE_STRUC_P) kmalloc(PROFILE_SIZE * sizeof(BATTERY_PROFILE_STRUC), GFP_KERNEL);
	memset(temp_profile_p, 0, PROFILE_SIZE * sizeof(*temp_profile_p));
	for (i=0; i< TOTAL_TEMP_MAP_NUM; i++) {
		profile_index = 0;
		for (j=0; j*2 <= 100; j++) {
			...
			bm_print(BM_LOG_FULL, "new battery_profile[%d,%d] <%d,%d>\n", i, j, (temp_profile_p + j)->percentage, (temp_profile_p + j)->voltage);
		}
        memcpy(profile_p[i], temp_profile_p, PROFILE_SIZE * sizeof(BATTERY_PROFILE_STRUC));
...
{
	BATTERY_PROFILE_STRUC_P low_profile_p = NULL, high_profile_p = NULL;
    ...
    if( temperature < gTemperature_map_tbl[0] ){
		low_profile_p = fgauge_get_profile(gTemperature_map_tbl[0]);
        memcpy( temp_profile_p, low_profile_p, saddles * sizeof(BATTERY_PROFILE_STRUC));

    }else if( temperature > gTemperature_map_tbl[TOTAL_TEMP_MAP_NUM - 1] ){
		low_profile_p = fgauge_get_profile(gTemperature_map_tbl[TOTAL_TEMP_MAP_NUM - 1]);
        memcpy( temp_profile_p, low_profile_p, saddles * sizeof(BATTERY_PROFILE_STRUC));
        ...
        for( i = 1; i < TOTAL_TEMP_MAP_NUM; i++){
            if( temperature <= gTemperature_map_tbl[i] ) {
                ...
                if( temperature == gTemperature_map_tbl[i-1] ){
                    low_profile_p = fgauge_get_profile(gTemperature_map_tbl[i-1]);
                    memcpy( temp_profile_p, low_profile_p, saddles * sizeof(BATTERY_PROFILE_STRUC));

                }else if( temperature == gTemperature_map_tbl[i] ){
                    low_profile_p = fgauge_get_profile(gTemperature_map_tbl[i]);
                    memcpy( temp_profile_p, low_profile_p, saddles * sizeof(BATTERY_PROFILE_STRUC));
                    ...
                    low_profile_p = fgauge_get_profile(gTemperature_map_tbl[i-1]);
                    high_temperature = gTemperature_map_tbl[i];
                    high_profile_p = fgauge_get_profile(gTemperature_map_tbl[i]);
                }
                break;
            }
        }
	...
	for (i = 0; i < saddles; i++) {
		bm_print(BM_LOG_FULL, "<DOD,Voltage> at %d = <%d,%d>\r\n",
...
{
	R_PROFILE_STRUC_P low_profile_p = NULL, high_profile_p = NULL;
	kal_int32 low_temperature, high_temperature;
	int i, saddles;
	kal_int32 temp_v_1 = 0, temp_v_2 = 0;
    ...
    if( temperature < gTemperature_map_tbl[0] ){
		low_profile_p = fgauge_get_profile_r_table(gTemperature_map_tbl[0]);
        memcpy( temp_profile_p, low_profile_p, saddles * sizeof(R_PROFILE_STRUC));

    }else if( temperature > gTemperature_map_tbl[TOTAL_TEMP_MAP_NUM - 1] ){
		low_profile_p = fgauge_get_profile_r_table(gTemperature_map_tbl[TOTAL_TEMP_MAP_NUM - 1]);
        memcpy( temp_profile_p, low_profile_p, saddles * sizeof(R_PROFILE_STRUC));
        ...
        for( i = 1; i < TOTAL_TEMP_MAP_NUM; i++){
            if( temperature <= gTemperature_map_tbl[i] ) {
                ...
                if( temperature == gTemperature_map_tbl[i-1] ){
                    low_profile_p = fgauge_get_profile_r_table(gTemperature_map_tbl[i-1]);
                    memcpy( temp_profile_p, low_profile_p, saddles * sizeof(R_PROFILE_STRUC));

                }else if( temperature == gTemperature_map_tbl[i] ){
                    low_profile_p = fgauge_get_profile_r_table(gTemperature_map_tbl[i]);
                    memcpy( temp_profile_p, low_profile_p, saddles * sizeof(R_PROFILE_STRUC));
                    ...
                    low_profile_p = fgauge_get_profile_r_table(gTemperature_map_tbl[i-1]);
                    high_temperature = gTemperature_map_tbl[i];
                    high_profile_p = fgauge_get_profile_r_table(gTemperature_map_tbl[i]);
                }
                break;
            }
        }
	...
	for (i = 0; i < saddles; i++) {
		bm_print(BM_LOG_FULL, "<Rbat,VBAT> at %d = <%d,%d>\r\n",
...
extern void mt6332_bat_int_check_setting(kal_int32 temp);
void fgauge_construct_table_by_temp(void)
{
	...
	if (avg_temp != last_temp) {
        ...
        gFG_BATT_CAPACITY_aging = fgauge_get_Q_max(curr_temp) + gFG_BATT_CAPACITY_aging_offset;
        mt6332_bat_int_check_setting(curr_temp);
		last_temp = avg_temp;
...
void fg_qmax_update_for_aging(void)
{
	...
	if (hw_charging_done == KAL_TRUE)	
	{
		if (gFG_DOD0 > 85) {
			if (gFG_columb < 0)
				gFG_columb = gFG_columb - gFG_columb * 2;	
			...
			if (gFG_BATT_CAPACITY_aging == 0) {
                ...
                gFG_BATT_CAPACITY_aging_offset = gFG_BATT_CAPACITY_aging - fgauge_get_Q_max(force_get_tbat(FALSE));
            }

			bm_print(BM_LOG_CRTI,
				 "[fg_qmax_update_for_aging] need update : gFG_columb=%d, gFG_DOD0=%d, new_qmax=%d, new_qmax_off=%d\r\n",
				 gFG_columb, gFG_DOD0, gFG_BATT_CAPACITY_aging, gFG_BATT_CAPACITY_aging_offset);
		} else {
			bm_print(BM_LOG_CRTI,
				 "[fg_qmax_update_for_aging] no update : gFG_columb=%d, gFG_DOD0=%d, new_qmax=%d, new_qmax_off=%d\r\n",
				 gFG_columb, gFG_DOD0, gFG_BATT_CAPACITY_aging, gFG_BATT_CAPACITY_aging_offset);
		}
	} else {
...
void dod_init(void)
{
	kal_bool use_rtc_fg = KAL_TRUE;
	kal_int32 bat_vol;
#if defined(SOC_BY_HW_FG)
	int ret = 0;
    int hw_ocv_tune_value = 35;
	
	ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &gFG_voltage);
    gFG_voltage += hw_ocv_tune_value;  
    gFG_hwocv_init = gFG_voltage;
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
    gFG_capacity_by_hwocv_init = gFG_capacity_by_v;
...
#endif
	{
		if (g_rtc_fg_soc == 1) {
			...
			if((bat_is_charger_exist() == KAL_TRUE) && (bat_vol > 3600))
				use_rtc_fg = KAL_FALSE;
			if((bat_is_charger_exist() == KAL_FALSE) && (bat_vol > 3500))
				use_rtc_fg = KAL_FALSE;
		}

		if (use_rtc_fg == KAL_TRUE) {
			bm_print(BM_LOG_CRTI, "[FGADC] boot with g_rtc_fg_soc");
...
void oam_init(void)
{
	...
	gFG_BATT_CAPACITY_aging = fgauge_get_Q_max(force_get_tbat(KAL_FALSE)) + gFG_BATT_CAPACITY_aging_offset;
...
kal_int32 fgauge_compensate_battery_voltage(kal_int32 ori_voltage)
{
	...
	gFG_resistance_bat = fgauge_read_r_bat_by_v(ori_voltage);	
...
kal_int32 fgauge_get_dod0(kal_int32 voltage, kal_int32 temperature, kal_bool bOcv)
{
	...
	if (bOcv == KAL_FALSE) {
		...
		voltage = voltage + fgauge_compensate_battery_voltage_recursion(voltage, 5);	
...
kal_int32 fgauge_update_dod(void)
{
	...
	bm_print(BM_LOG_FULL, "[fgauge_update_dod] FG_dod_1=%d, adjust_coulomb_counter=%d, gFG_columb=%d, gFG_DOD0=%d, gFG_temp=%d, gFG_BATT_CAPACITY=%d, C_0mA=%d, C_400mA=%d, C_FGCurrent=%d, gFG_current_AVG=%d\n", 
...
kal_int32 fgauge_read_capacity(kal_int32 type)
{
	...
	if (type == 0)		
	{
		...
		dvalue = fgauge_get_dod0(voltage, temperature, KAL_FALSE);	
	} else {
		
		dvalue = fgauge_update_dod();	
	}
...
void fgauge_algo_run(void)
{
	...
	gFG_voltage = gFG_voltage + fgauge_compensate_battery_voltage_recursion(gFG_voltage, 5);	
	...
	if (columb_delta < 0)
		columb_delta = columb_delta - 2 * columb_delta;	
	...
	bm_print(BM_LOG_CRTI,
		 "[FGADC] Curr: %c%d,CC: %d,Volt: %d,SOC_by_v: %d,(rtc)%d,(hw)%d,%d,(sw)%d,%d,SOC: %d,%d,%d,Caging: %d,%d,%d,%d,%d,%d,%d,%d,D0: %d,D1: %d,%d,%d\r\n",
		 gFG_Is_Charging>0? '+':'-' , gFG_current, gFG_columb, gFG_voltage,
		 gFG_capacity_by_v, g_rtc_fg_soc, gFG_capacity_by_hwocv_init, gFG_hwocv_init, gFG_capacity_by_v_init, gFG_swocv_init,
		 gFG_capacity_by_c, gFG_capacity_by_c_init, gFG_BATT_CAPACITY,
		 gFG_BATT_CAPACITY_aging, gFG_BATT_CAPACITY_aging_offset, gFG_compensate_value, gFG_ori_voltage,
		 OCV_BOARD_COMPESATE, R_FG_BOARD_SLOPE, gFG_voltage_init, MinErrorOffset,
		 gFG_DOD0, gFG_DOD1, CAR_TUNE_VALUE, AGING_TUNING_VALUE);
...
void fgauge_initialization(void)
{
	...
	gFG_temp = force_get_tbat(KAL_FALSE);
    mt6332_bat_int_check_setting(gFG_temp);
	...
	gFG_BATT_CAPACITY_aging = fgauge_get_Q_max(gFG_temp) + gFG_BATT_CAPACITY_aging_offset;
...
kal_int32 get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	else if (vbat_val > VBAT_LOW_POWER_WAKEUP)	
		g_spm_timer = LOW_POWER_WAKEUP_PERIOD;	
	else
	...
	if (vbat_val > VBAT_NORMAL_WAKEUP)	
	{
		...
		if (g_chg_limit_reason & HTC_BATT_CHG_LIMIT_BIT_TALK)
			g_spm_timer = LOW_POWER_WAKEUP_PERIOD;	
		else if (g_spm_timer > NORMAL_WAKEUP_PERIOD)
			g_spm_timer = NORMAL_WAKEUP_PERIOD; 
...
{
	 int ret;
	 kal_int32 ADC_I_SENSE=1;   
	 kal_int32 ADC_BAT_SENSE=1;	
 	 int ICharging=0;
...
kal_int32 battery_meter_get_battery_temperature(kal_bool update)
{
#ifdef MTK_BATTERY_LIFETIME_DATA_SUPPORT
...
kal_int32 battery_meter_reset(kal_uint32 uiUI_soc)
{
#if defined(CONFIG_POWER_EXT)
	return 0;
#else

	if (bat_is_charging_full() == KAL_TRUE)	
	{
...
kal_uint32 htc_battery_meter_get_battery_c_aging(void)
{
...
{
    if( battery_profile_temperature != NULL ){
        kfree(battery_profile_temperature);
        battery_profile_temperature = NULL;
    }

    if( r_profile_temperature != NULL ){
        kfree(r_profile_temperature);
        r_profile_temperature = NULL;
    }
...
static void battery_aging_check(void)
{
...
#endif
	{
        ...
        soc_after_sleep = fgauge_read_capacity_by_v(hw_ocv_after_sleep);
        htc_battery_update_gap( bat_get_ui_percentage() - soc_after_sleep );
        ret = battery_meter_ctrl(BATTERY_METER_CMD_HW_RESET, NULL);
        gFG_columb = 0;
        reset_parameter_dod_full(soc_after_sleep);
        bm_print(BM_LOG_CRTI, "[aging check] HW OCV update, soc = %d\n", soc_after_sleep);
	}
...
static void htc_battery_meter_cc_sync(void)
{
    kal_int32 iCapacity_by_c_tmp, iCapcity_by_c_after, iUI_soc_ori, iGap;
    kal_int32 ret;

    iCapacity_by_c_tmp = gFG_capacity_by_c;
    iUI_soc_ori = bat_get_ui_percentage();
    ret = battery_meter_get_battery_voltage(KAL_TRUE);  
    iCapcity_by_c_after = battery_meter_get_battery_percentage();
    iGap = iCapacity_by_c_tmp - iCapcity_by_c_after;
    if( iGap > 0 )
        bat_set_ui_percentage( (iUI_soc_ori-iGap > 0)? (iUI_soc_ori - iGap) : 0 );

    bm_print(BM_LOG_CRTI, "[%s] iGap = %d, soc = %d, ui_soc = %d\n",
        __FUNCTION__, iGap, iCapcity_by_c_after, bat_get_ui_percentage());
}
#endif
...
#if defined(SOC_BY_SW_FG)    
	battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &hw_ocv_after_sleep);
	if (_g_bat_sleep_total_time > 3600) {	
		if (hw_ocv_after_sleep < g_hw_ocv_before_sleep) {
...
static int battery_meter_dts_probe(struct platform_device *dev)
{
	...
	bm_print(BM_LOG_CRTI, "******** battery_meter_dts_probe!! ********\n");
