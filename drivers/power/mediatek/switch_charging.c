...
extern kal_bool set_current_done;
...
static void set_ta_charging_current(void)
{
	...
	if((real_v_chrA - ta_v_chr_org) > 3000) {
		g_temp_input_CC_value = TA_AC_9V_INPUT_CURRENT;  
		g_temp_CC_value = TA_AC_CHARGING_CURRENT;
	} else if((real_v_chrA - ta_v_chr_org) > 1000) {
		g_temp_input_CC_value = TA_AC_7V_INPUT_CURRENT;  
		g_temp_CC_value = TA_AC_CHARGING_CURRENT;
	}
}
...
bool get_usb_current_unlimited(void)
{
	if (BMT_status.charger_type == STANDARD_HOST || BMT_status.charger_type == CHARGING_HOST
		|| BMT_status.charger_type == CHARGER_UNKNOWN || BMT_status.charger_type == NONSTANDARD_CHARGER)
...
void select_charging_curret(void)
{
	...
	} else if (g_chg_in_mhl == CONNECT_TYPE_MHL_500MA) {
		g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;
...
static void pchr_turn_on_charging(void)
{
	...
	static kal_uint32 i_aicl_done = 0;
	static CHR_CURRENT_ENUM pre_g_temp_CC_value = 0;
	static CHR_CURRENT_ENUM pre_g_temp_input_CC_value = 0;
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA
            ...
            if( BMT_status.aicl_state == AICL_STOP ){
                ...
                if( g_temp_CC_value == AC_CHARGER_CURRENT && BMT_status.bat_vol < 4000 )
                    battery_charging_control(CHARGING_CMD_HTC_AICL_INIT, &BMT_status);
                ...
                if( (BMT_status.charger_type == STANDARD_CHARGER) && g_temp_CC_value == AC_CHARGER_CURRENT && BMT_status.bat_vol < 4000 ){
                    BMT_status.aicl_state = AICL_START;
            ...
            }else if( BMT_status.aicl_state == AICL_DONE ){

                battery_charging_control(CHARGING_CMD_GET_CURRENT, &i_chr);
				battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] i_chr = %d, g_chg_limit_reason = %d, i_aicl_done = %d",
                        i_chr, g_chg_limit_reason, i_aicl_done);
                if( g_temp_CC_value == AC_CHARGER_CURRENT ){
                    if( (i_chr < AC_CHARGER_CURRENT) && (i_aicl_done == 0) ){
                        battery_xlog_printk(BAT_LOG_CRTI,
                                "[BATTERY] I-chr is set to %dmA, return i-chr back to 1A.\n", i_chr/100);
                        set_current_done = KAL_FALSE;
                        battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,
                                        &g_temp_input_CC_value);
                        battery_charging_control(CHARGING_CMD_SET_CURRENT, &g_temp_CC_value);
						...
						if (g_chg_limit_reason & HTC_BATT_CHG_LIMIT_BIT_THRML) {
							if (i_chr > AC_CHARGER_CURRENT) {
								i_aicl_done = i_chr;
								i_chr = AC_CHARGER_CURRENT;
								set_current_done = KAL_FALSE;
								battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT, &i_chr);
								battery_charging_control(CHARGING_CMD_SET_CURRENT, &i_chr);
							}
				        } else if (g_chg_limit_reason == 0 && g_bcct_flag == 0) {
					        if (i_aicl_done != 0) {
                                set_current_done = KAL_FALSE;
						        battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT, &i_aicl_done);
						        battery_charging_control(CHARGING_CMD_SET_CURRENT, &i_aicl_done);
                                i_aicl_done = 0;
					        } else {
                                battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT, &i_chr);
                                battery_charging_control(CHARGING_CMD_SET_CURRENT, &i_chr);
                    ...
                    if ((g_chg_limit_reason & HTC_BATT_CHG_LIMIT_BIT_TALK) ||
						...
						(g_bcct_flag == 1)) {
                        if ((i_chr > g_temp_CC_value) && (i_aicl_done == 0)) {
                            i_aicl_done = i_chr;
                            i_chr = g_temp_CC_value;
                            set_current_done = KAL_FALSE;
                        }
                    }
                    battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,
