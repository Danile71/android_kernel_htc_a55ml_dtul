...
#if defined(A55ML_BATT)
	if((BMT_status.temperature < -10) && (BMT_status.bat_vol < 3800)
		&& (fgauge_get_battery_id() == 2) && g_display_on) {
...
static kal_uint32 htc_battery_0percent_volt(kal_int32 temp)
{
    kal_int32 iGap_temp_map_tbl[] = {0, 10};
#if defined(A55ML_BATT)
	kal_uint32 ui0percent_volt_map_tbl[] = {2950, 3300, 3300};
#else
...
static void htc_batt_late_resume(struct early_suspend *h)
{
	...
	if((BMT_status.temperature < -10) && (BMT_status.bat_vol < 3800)
		&& (fgauge_get_battery_id() == 2)) {
			battery_xlog_printk(BAT_LOG_CRTI, "[%s] Notify battery level 0.\n", __FUNCTION__);
			wake_up_bat();
	}
#endif
	return;
