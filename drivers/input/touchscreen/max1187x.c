...
#endif
#ifdef MTK_PLATFORM
#define pr_info(fmt, ...) printk(pr_fmt(fmt) "\n", ##__VA_ARGS__)
#else
...
#endif
#ifdef MTK_PLATFORM
#define pr_err(fmt, ...) printk(MAX1187X_LOG_NAME \
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (value) {
#ifdef MTK_PLATFORM
		ret = irq_registration(&ts->client->irq, irq_handler,
...
static struct max1187x_pdata *max1187x_get_platdata_dt(struct device *dev)
{
	...
	if (of_property_read_u32(devnode, "gpio_3v3", &pdata->gpio_3v3)) {
		pr_info("No property: gpio_3v3\n");
	}

	if (of_property_read_u32(devnode, "gpio_switch", &pdata->gpio_switch)) {
	...
	if (of_property_read_bool(devnode, "switch_invert")) {
		pr_info("switch pin invert");
		pdata->switch_invert = true;
	} else {
		pr_debug("No property: switch_invert");
	...
	if(of_property_read_bool(devnode, "no_force_calibration")) {
		pr_info("No need force calibration");
		pdata->no_force_calibration = true;
	} else {
	...
	if (of_property_read_bool(devnode, "disp_panel_check")) {
		scnprintf(pdata->disp_panel, ARRAY_SIZE(pdata->disp_panel), mtkfb_find_lcm_driver());
	...
	if (value) {
		...
		if (gpio_is_valid(pdata->gpio_reset)) {
			...
			if (gpio_is_valid(pdata->gpio_reset)) {
				...
				mt_set_gpio_out(pdata->gpio_reset, 1);
				mdelay(50);
			}
		}
		...
		mt_set_gpio_mode(pdata->gpio_tirq, GPIO_CTP_EINT_PIN_M_EINT);
		mt_set_gpio_dir(pdata->gpio_tirq, GPIO_DIR_IN);
		mt_set_gpio_pull_enable(pdata->gpio_tirq, GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(pdata->gpio_tirq, 1);
		msleep(50);
#else
		if (gpio_is_valid(pdata->gpio_switch)) {
			ret = gpio_request(pdata->gpio_switch, "max1187x_switch");
...
#ifdef MTK_PLATFORM
	if (irq_registration(&ts->client->irq, irq_handler,
...
static int check_chip_exist(struct i2c_client *client)
{
	...
	for (i=0; i<RETRY_TIMES; i++) {
		...
		buf[1] = 0x00;
#if GTP_SUPPORT_I2C_DMA
		memcpy(gpDMABuf_va, buf, 2);
		if (i2c_master_send(client, gpDMABuf_pa, 2) <= 0)
#else
		if (i2c_master_send(client, buf, 2) <= 0)
#endif
			continue;
#if GTP_SUPPORT_I2C_DMA
		if (i2c_master_recv(client, gpDMABuf_pa, 2) <= 0)
#else
		if (i2c_master_recv(client, buf, 2) <= 0)
#endif
			continue;
#if GTP_SUPPORT_I2C_DMA
		memcpy(buf, gpDMABuf_va, 2);
#endif
		read_len = (buf[0] + 1) << 1;
		if (read_len>10)
			read_len = 10;
#if GTP_SUPPORT_I2C_DMA
		if (i2c_master_recv(client, gpDMABuf_pa, read_len) <= 0)
#else
		if (i2c_master_recv(client, buf, read_len) <= 0)
#endif
			continue;
#if GTP_SUPPORT_I2C_DMA
		memcpy(buf, gpDMABuf_va, read_len);
#endif
		break;
	}
	if (i == RETRY_TIMES) {
...
#if GTP_SUPPORT_I2C_DMA
	memcpy(gpDMABuf_va, data, len);
	client->addr = client->addr & I2C_MASK_FLAG;
	client->addr = client->addr | I2C_DMA_FLAG;

	do {ret = i2c_master_send(client, (char *) gpDMABuf_pa, (int) len);
#else
...
static void switch_sensor_hub(struct data *ts, int mode)
{
	...
	if (gpio_is_valid(PDATA(gpio_switch))) {
		...
		case 0:
			touch_status(0 | mask);
#ifdef MTK_PLATFORM
			if (PDATA(switch_invert))
				gpio_set_value(PDATA(gpio_switch), 1);
			else
				gpio_set_value(PDATA(gpio_switch), 0);
#else
			if (PDATA(switch_invert))
