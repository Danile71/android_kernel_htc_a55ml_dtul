...
void msdc_set_smpl(struct msdc_host *host, u8 HS400, u8 mode, u8 type, u8 *edge)
{
	...
	case TYPE_CMD_RESP_EDGE:
		if (HS400) {
			...
			sdr_set_field(EMMC50_CFG0, MSDC_EMMC50_CFG_CMDRSP_SEL, 0);	
		}
	...
	case TYPE_WRITE_CRC_EDGE:
		if (HS400) {
			sdr_set_field(EMMC50_CFG0, MSDC_EMMC50_CFG_CFCSTS_SEL, 1);	
		} else {
			sdr_set_field(EMMC50_CFG0, MSDC_EMMC50_CFG_CFCSTS_SEL, 0);	
		}
	...
	if (sd_18 && ((host->id == 1) || (host->id == 2))) {
		if (host->mmc->ios.timing == MMC_TIMING_UHS_SDR104) {
			sdr_set_field(MSDC_GPIO_ADDR(msdc_clk_ctrl_reg_offset[host->id]),
				GPIO_MSDC_DRV_MASK, hw->clk_drv_sd_18_sdr104);
			sdr_set_field(MSDC_GPIO_ADDR(msdc_cmd_ctrl_reg_offset[host->id]),
				GPIO_MSDC_DRV_MASK, hw->cmd_drv_sd_18_sdr104);
			sdr_set_field(MSDC_GPIO_ADDR(msdc_dat_ctrl_reg_offset[host->id]),
				GPIO_MSDC_DRV_MASK, hw->dat_drv_sd_18_sdr104);
		} else {
			sdr_set_field(MSDC_GPIO_ADDR(msdc_clk_ctrl_reg_offset[host->id]),
...
static void msdc_emmc_power(struct msdc_host *host, u32 on)
{
	...
	msdc_ldo_power(1, MT6331_POWER_LDO_VEMC33, VOL_3300, &g_msdc0_flash);
...
static void msdc_tasklet_card(unsigned long arg)
{
	...
	if (!host->suspend && host->sd_cd_insert_work) {
		...
		host->mmc->removed_cnt = 0;
		host->mmc->crc_count = 0;
		host->mmc->card_err_count = 0;
...
int msdc_get_reserve(void)
{
	...
	l_mtk_reserve = (unsigned int)(lp_excel_info->start_address & 0xFFFFUL) << 8;	
	...
	l_otp_reserve = (unsigned int)(lp_excel_info->start_address & 0xFFFFUL) << 8;	
...
static unsigned int msdc_command_start(struct msdc_host *host, struct mmc_command *cmd, int tune,	
...
static void msdc_restore_info(struct msdc_host *host)
{
	...
	sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL, host->saved_para.int_dat_latch_ck_sel);	
	sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_CKGEN_MSDC_DLY_SEL, host->saved_para.ckgen_msdc_dly_sel);	
	sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_CMD_RSP_TA_CNTR, host->saved_para.cmd_resp_ta_cntr);	
...
#if 0
			hs400_src = 0;	
			msdc_clock_src[host->id] = MSDC_CLKSRC_800MHZ;	
...
static struct msdc_reg_control hs400_backup_reg_list[HS400_BACKUP_REG_NUM] = {
	...
	{(OFFSET_MSDC_PATCH_BIT1), (MSDC_PB1_WRDAT_CRCS_TA_CNTR), 0x0, 0x0, hs400_restore_pb1},	
	{(OFFSET_MSDC_IOCON),
...
static void msdc_tune_async_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	host->mrq = mrq;
	do {
		ERR_MSG("%s() tuning count:%d", __func__, err_count);
		msdc_dump_trans_error(host, cmd, data, stop, sbc);	
		...
		if (data || (((cmd->opcode == MMC_SWITCH)  || (cmd->opcode == MMC_SET_WRITE_PROT)
			...
			if (host->hw->host_function == MSDC_SD) {
				goto sd_err_retry;
			} else if (msdc_abort_data(host))
				ERR_MSG("abort failed");
		}
		
		if (cmd->error == (unsigned int)-ETIMEDOUT) {
			if (host->hw->host_function == MSDC_SD) {
				ERR_MSG("msdc%d: CMD%d timeout", host->id, cmd->opcode);
				goto sd_err_retry;
			}
		...
		if (data && (data->error == (unsigned int)-ETIMEDOUT)) {
			if (data->flags & MMC_DATA_READ) {
				if (!(host->sw_timeout) && (host->hw->host_function == MSDC_SD)) {
					ERR_MSG("execute Read-tune, SW timeout: %d", host->sw_timeout);
					msdc_tune_read(host);
				}
			} else if (data->flags & MMC_DATA_WRITE) {
				if (!(host->sw_timeout) && (host->hw->host_function == MSDC_SD)) {
					ERR_MSG("execute Write-tune, SW timeout: %d", host->sw_timeout);
					msdc_tune_write(host);
				} else if (!(host->sw_timeout) &&
					   (host->hw->host_function == MSDC_EMMC) &&
					   (host->write_timeout_emmc < MSDC_MAX_W_TIMEOUT_TUNE_EMMC)) {
...
sd_err_retry:
 ...
 out:
	if (host->hw->host_function == MSDC_SD) {
		if (err_count >= HTC_MAX_RETRY_COUNT) {
			ERR_MSG("%s() Tuning fail (count:%d)", __func__, err_count);
			mmc_card_set_removed(host->mmc->card);
		} else {
			ERR_MSG("%s() Tuning end (insert:%d, count:%d)"
				, __func__, is_card_present(host), count);
...
static int msdc_ops_switch_volt(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->signal_voltage != MMC_SIGNAL_VOLTAGE_330) {
		...
		if ((sdr_read32(MSDC_PS) & ((1 << 24) | (0xF << 16))) == 0) {
			...
			msdc_set_mclk(host, MSDC_STATE_DEFAULT, 260000);
...
static void msdc_underclocking(struct msdc_host *host)
{
	...
	case MMC_TIMING_UHS_SDR12:
		...
		msdc_host_mode[host->id] &= ~MMC_CAP_UHS_SDR12;
	case MMC_TIMING_UHS_SDR25:
		host->mmc->caps &= ~MMC_CAP_UHS_SDR25;
		msdc_host_mode[host->id] &= ~MMC_CAP_UHS_SDR25;
	case MMC_TIMING_UHS_SDR50:
		host->mmc->caps &= ~MMC_CAP_UHS_SDR50;
		msdc_host_mode[host->id] &= ~MMC_CAP_UHS_SDR50;
	case MMC_TIMING_UHS_DDR50:
		host->mmc->caps &= ~MMC_CAP_UHS_DDR50;
		msdc_host_mode[host->id] &= ~MMC_CAP_UHS_DDR50;
	case MMC_TIMING_UHS_SDR104:
		host->mmc->caps &= ~MMC_CAP_UHS_SDR104;
		msdc_host_mode[host->id] &= ~MMC_CAP_UHS_SDR104;
		break;
	default:
...
static int msdc1_read_tray_status(struct seq_file *m, void *v)
{
	...
	return seq_printf(m, "%d", msdc_ops_get_cd(host));
}
...
{
   msdc_ldo_power(0, MT6331_POWER_LDO_VMC, VOL_3300, &g_msdc1_io);
   msdc_ldo_power(0, MT6331_POWER_LDO_VMCH, VOL_3300, &g_msdc1_flash);
   pr_info("[%s]: power off ldo vmc and vmch done.\n", __func__);
}
	...
	if ((ghost != NULL) && (ghost->hw->flags & MSDC_SDIO_IRQ)) {
		...
		if (src_clk_control) {
			...
			if (ghost->mmc->sdio_irq_thread &&
				 (atomic_read(&ghost->mmc->sdio_irq_thread_abort) == 0)) {
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (host->id == 1) {
		...
		host->sd_tray_state = proc_create_data("sd_tray_state", 0444, NULL,
						&msdc1_proc_sd_tray_fops, host->mmc);
	...
	if (is_card_sdio(host) || (host->hw->flags & MSDC_SDIO_IRQ)) {
		...
		if (host->saved_para.suspend_flag == 0) {
			...
			if (host->saved_para.hz) {
				...
				sdr_get_field(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL, host->saved_para.int_dat_latch_ck_sel);	
				sdr_get_field(MSDC_PATCH_BIT0, MSDC_PB0_CKGEN_MSDC_DLY_SEL, host->saved_para.ckgen_msdc_dly_sel);	
				sdr_get_field(MSDC_PATCH_BIT1, MSDC_PB1_CMD_RSP_TA_CNTR, host->saved_para.cmd_resp_ta_cntr);	
				sdr_get_field(MSDC_PATCH_BIT1, MSDC_PB1_WRDAT_CRCS_TA_CNTR, host->saved_para.wrdat_crc_ta_cntr);	
				...
				host->saved_para.vcore_uv = autok_get_current_vcore_offset();
			}
