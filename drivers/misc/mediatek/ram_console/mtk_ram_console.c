...
#include <linux/htc_bootloader_log.h>
#endif
...
struct last_reboot_reason {
	...
	void *kparams;
#ifdef CONFIG_HTC_DEBUG_POWERDOWN
	uint32_t powerdown_reason;
	uint64_t powerdown_time_utc;
#endif
#ifdef CONFIG_HTC_DEBUG_FOOTPRINT
	uint32_t cpu_footprint[NR_CPUS];
	uint32_t cpu_freq;
	uint32_t dvfs_vproc_volt;
	uint32_t dvfs_vsram_volt;
#endif
};
...
#define REBOOT_REASON_SIG (0x43474244)	
...
#ifdef CONFIG_HTC_DEBUG_FOOTPRINT
#define CUR_RRR_VAL(rr_item) LAST_RR_SEC_VAL(ram_console_buffer, linux, struct last_reboot_reason, rr_item)
#define CUR_RRPL_VAL(rr_item) LAST_RR_SEC_VAL(ram_console_buffer, pl, struct reboot_reason_pl, rr_item)
#endif
...
{
	struct rtc_time tm;
	seq_printf(m, "Power down reason: %d\n", LAST_RR_SEC_VAL(bufp, linux, struct last_reboot_reason, powerdown_reason));

	rtc_time_to_tm(LAST_RR_SEC_VAL(bufp, linux, struct last_reboot_reason, powerdown_time_utc), &tm);
	seq_printf(m, "Power down time: %d-%02d-%02d %02d:%02d:%02d UTC\n",
...
void sram_log_save(const char *msg, int count)
{
	...
	if (count >= ram_console_buffer_size) {
		...
		buffer->start = 0;
		buffer->size = ram_console_buffer_size;
	} else if (count > (ram_console_buffer_size - buffer->start))	
	{
	...
	{
		memcpy(rc_console + buffer->start, msg, count);	
		buffer->start += count;
		buffer->size += count;
		if (buffer->start >= ram_console_buffer_size) {
			buffer->start = 0;
...
static void __init ram_console_save_old(struct ram_console_buffer *buffer)
{
	...
	if (buffer->bin_log_count == 0) {
		...
		bin_log_size = buffer->bin_log_count * 5 / 4;	
...
static int __init ram_console_init(struct ram_console_buffer *buffer, size_t buffer_size)
{
	...
	if (buffer->off_console != 0 && buffer->off_linux + ALIGN(sizeof(struct last_reboot_reason), 64) == buffer->off_console) {
		...
		pr_err("[HTC] ram_console: off_console:%x\n", buffer->off_console);

                if((buffer->off_console + buffer->size)> CONFIG_MTK_RAM_CONSOLE_DRAM_SIZE )
		{
                        pr_err("[HTC] ram_console: buffer erorr detected, buffer size too big.(0x%x+0x%x>0x%x), recover it.\n",
                                buffer->off_console, buffer->size, CONFIG_MTK_RAM_CONSOLE_DRAM_SIZE);

                        buffer->size = CONFIG_MTK_RAM_CONSOLE_DRAM_SIZE - buffer->off_console;
                }

		if(buffer->start > buffer->size)
		{
			pr_err("[HTC] ram_console: buffer erorr detected, start out of bound.(0x%x>=0x%x), recover it.\n",
...
static int __init ram_console_late_init(void)
{
...
#if defined(CONFIG_HTC_DEBUG_BOOTLOADER_LOG)
	ram_console_old_log = kmalloc(ram_console_old_log_size + str_real_len + bl_last_log_buf_size + bl_cur_log_buf_size, GFP_KERNEL);
#else
	...
	if(bl_last_log_buf == NULL || bl_last_log_buf_size == 0) {
		pr_err("ram_console: failed to get bl last log buf\n");
		return 0;
	}
	memcpy(ram_console_old_log + str_real_len,
	       bl_last_log_buf, bl_last_log_buf_size);
	memcpy(ram_console_old_log + str_real_len + bl_last_log_buf_size ,
	       ram_console_old_log_init_buffer, ram_console_old_log_size);
	if(bl_cur_log_buf == NULL || bl_cur_log_buf_size == 0) {
		pr_err("ram_console: failed to get bl cur log buf\n");
		return 0;
	}
	memcpy(ram_console_old_log + str_real_len + bl_last_log_buf_size + ram_console_old_log_size,
...
#if defined(CONFIG_HTC_DEBUG_BOOTLOADER_LOG)
	ram_console_old_log_size += bl_last_log_buf_size;
	ram_console_old_log_size += bl_cur_log_buf_size;
	bldr_log_release();
#endif
	return 0;
}
...
#ifdef CONFIG_HTC_DEBUG_POWERDOWN
void set_powerdown_reason(u32 reason)
{
	LAST_RR_SET(powerdown_reason, reason);
	mb();
}

void set_powerdown_pet_time(unsigned long long time)
{
	LAST_RR_SET(powerdown_time_utc, time);
	mb();
}
#endif

#ifdef CONFIG_HTC_DEBUG_FOOTPRINT
void set_cpu_footprint(unsigned cpu, unsigned state)
{
	unsigned mask = 0xFF;
	unsigned new_state;

	if (unlikely(cpu >= NR_CPUS)) {
		pr_err("Only %d cores, but try to set footprint for core %d\n", NR_CPUS, cpu);
		return;
	}

	LAST_RR_SET_WITH_ID(cpu_footprint, cpu, CPU_FOOT_PRINT_MAGIC_HOTPLUG | state);
	mb();
}
#endif
...
#ifdef CONFIG_HTC_DEBUG_FOOTPRINT
void aee_rr_rec_cpu_freq(u32 val)
{
	LAST_RR_SET(cpu_freq, val);
}

void aee_rr_rec_dvfs_vproc_volt(u32 val)
{
	LAST_RR_SET(dvfs_vproc_volt, val);
}

void aee_rr_rec_dvfs_vsram_volt(u32 val)
{
	LAST_RR_SET(dvfs_vsram_volt, val);
}
#endif
...
#ifdef CONFIG_HTC_DEBUG_FOOTPRINT
void aee_rr_show_cpu_freq(struct seq_file *m)
{
	seq_printf(m, "cpu_freq: %d KHz\n", LAST_RRR_VAL(cpu_freq));
}

void aee_rr_show_dvfs_vproc_volt(struct seq_file *m)
{
	seq_printf(m, "dvfs_vproc_volt: %d mv\n", LAST_RRR_VAL(dvfs_vproc_volt));
}

void aee_rr_show_dvfs_vsram_volt(struct seq_file *m)
{
	seq_printf(m, "dvfs_vsram_volt: %d mv\n", LAST_RRR_VAL(dvfs_vsram_volt));
}
#endif
...
#ifdef CONFIG_HTC_DEBUG_FOOTPRINT
void aee_rr_show_cpu_footprint(struct seq_file *m, int cpu)
{
	seq_printf(m, "  Cpu footprint: 0x%x\n", LAST_RRR_VAL(cpu_footprint)[cpu]);
}
#endif
...
#ifdef CONFIG_HTC_DEBUG_FOOTPRINT
	aee_rr_show_cpu_freq,
	aee_rr_show_dvfs_vproc_volt,
	aee_rr_show_dvfs_vsram_volt,
#endif
};
...
#ifdef CONFIG_HTC_DEBUG_FOOTPRINT
static int __init boot_init_cpu_foot_print(void)
{
	int i;

	
	for (i = 1; i < NR_CPUS; i++) {
		LAST_RR_SET_WITH_ID(cpu_footprint, i, CPU_FOOT_PRINT_MAGIC_HOTPLUG | 0x0);
	}
	mb();

	return 0;
}
early_initcall(boot_init_cpu_foot_print);
#endif
