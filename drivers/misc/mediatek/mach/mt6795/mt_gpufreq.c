...
#define GPU_DVFS_VOLT1     (112500)  
#define GPU_DVFS_VOLT2     (100000)  
#define GPU_DVFS_VOLT3     ( 90000)  

#define GPU_DVFS_CTRL_VOLT     (2)  

#define GPU_DVFS_PMIC_SETTLE_TIME (40) 
...
unsigned long htc_vgpu_vol_get(void)
{
	unsigned int RegValGet = 0;
	pmic_read_interface(MT6331_VDVFS13_CON12, &RegValGet, 0x7F, 0x0);

	return mt_gpufreq_pmic_wrap_to_volt(RegValGet) * 10;
}

int htc_vgpu_vol_set(int vgpu_uv)
{
	int i;
	...
	{
		mt_gpufreqs[i].gpufreq_volt = vgpu_uv;
	}

	mt_gpu_volt_switch(g_cur_gpu_volt, vgpu_uv);

	g_cur_gpu_volt = vgpu_uv;
...
unsigned int mt_gpufreq_voltage_enable_set(unsigned int enable)
{
	...
	if(enable == 1)
		pmic_config_interface(0x02AA, 0x1, 0x1, 0x0); 
	else
		pmic_config_interface(0x02AA, 0x0, 0x1, 0x0); 
...
static int mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	if(mt_gpufreq_dvfs_table_type == 0)
		...
		mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt_gpufreq_opp_tbl_1)); 
	else if(mt_gpufreq_dvfs_table_type == 2)
		mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt_gpufreq_opp_tbl_2)); 
	else
		mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt_gpufreq_opp_tbl_0));
#else
	if(mt_gpufreq_dvfs_table_type == 0)
		...
		mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt_gpufreq_opp_tbl_3)); 
	else if(mt_gpufreq_dvfs_table_type == 4)
		mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt_gpufreq_opp_tbl_4)); 
	else
		mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt_gpufreq_opp_tbl_0));
#endif

    pmic_config_interface(0x02A6, 0x1, 0x1, 0x1); 
	pmic_config_interface(0x02A6, 0x0, 0x1, 0x0); 
	pmic_config_interface(0x02AA, 0x1, 0x1, 0x0); 
    ...
    pmic_read_interface(0x02A6, &RegValGet, 0x1, 0x1); 
    xlog_printk(ANDROID_LOG_INFO, "Power/GPU_DVFS", "VDVFS13_VOSEL_CTRL[1] = %d\n", RegValGet);
	pmic_read_interface(0x02A6, &RegValGet, 0x1, 0x0); 
	xlog_printk(ANDROID_LOG_INFO, "Power/GPU_DVFS", "VDVFS13_EN_CTRL[0] = %d\n", RegValGet);
	pmic_read_interface(0x02AA, &RegValGet, 0x1, 0x0); 
