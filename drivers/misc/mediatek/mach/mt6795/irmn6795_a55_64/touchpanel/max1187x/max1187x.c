...
struct data {
	...
	char fw_ver[16];
	u8  protocol_ver;
	u16 fw_version[RPT_LEN_PACKET_MAX];

	u16 rx_report[RPT_LEN_MAX]; 
	u16 rx_report_len;
	u16 rx_packet[RPT_LEN_PACKET_MAX + 1]; 
	u32 irq_count;
	u16 framecounter;
	u16 list_finger_ids;
	u8 sysfs_created;
...
{
	u16 tx_buf[CMD_LEN_PACKET_MAX + 2]; 
...
static void tpd_down(s32 x, s32 y, s32 major, s32 min, s32 id, s32 pressure)
{
   ...
   input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, id);
    ...
    input_report_abs(tpd->dev, ABS_MT_POSITION_X, y);
			...
			char *buf)
{
	...
	rbcmd_send_receive(ts, cmd_buf, 2, 0x0140, HZ/4);

	ts->chip_id = BYTEH(ts->rbcmd_rx_report[4]);

	return snprintf(buf, PAGE_SIZE, "Maxim-%u.%u.%u" \
					"_chipID-0x%02X\n",
					BYTEH(ts->rbcmd_rx_report[3]),
					BYTEL(ts->rbcmd_rx_report[3]),
					ts->rbcmd_rx_report[5],
...
static int max1187x_auto_detect_check(struct data *ts)
{
	...
	while (i2c_tx_b(ts, buffer, 2) < 0) {
		TPD_DMESG("max1187x_auto_detect_check RX fail:try_count=%d\n",try_count);
	    try_count++;
		if(try_count <=10)continue;
...
static int max_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (android_touch_kobj == NULL) {
		TPD_DMESG("failed to create kobj\n");
		return 0;
	}
	...
	if (sysfs_create_link(android_touch_kobj, &client->dev.kobj, "maxim1187x") < 0) {
		TPD_DMESG("failed to create link\n");
		return 0;
	}
	...
	TPD_DMESG("max_i2c_probe done.countxxx = %d, flag = %d",count,check_flag);
...
static int tpd_local_init(void)
{
	...
	hwPowerOn(TPD_POWER_SOURCE_CUSTOM,VOL_2800,"CTP");
	mdelay(50);
#endif
