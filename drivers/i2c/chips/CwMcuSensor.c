...
#define COMPASS_DIALOG_DEBOUNCE 180000  
...
struct cwmcu_data {
    ...
    struct work_struct  mcu_irq_work;
	...
	unsigned long popup_compass_dialog_jiffies;
...
static void mcu_chip_mode_set(struct cwmcu_data *mcu_data, int onoff)
{
    if (mcu_data) {
#ifdef IS_MTK_PLATFORM
        mt_set_gpio_dir(mcu_data->gpio_chip_mode, 1);
        mt_set_gpio_mode(mcu_data->gpio_chip_mode, 0);
        mt_set_gpio_out(mcu_data->gpio_chip_mode, onoff);
#else
                                  ...
                                  const char *buf, size_t count)
{
        ...
        data_temp[2]= (mcu_data->ps_thd_add == 0) ? 0x64 : mcu_data->ps_thd_add;
		...
		const char *buf, size_t count)
{
	...
	if (data_temp == 6) {
		mcu_data->rv_calibration = 1;
		I("%s: Writing, reg_addr = 0x%x, value = 0x%x\n", __func__,
		  CW_I2C_REG_RV_CALIBRATION, mcu_data->rv_calibration);
		error = CWMCU_i2c_write_power(s_mcu_data,
					   CW_I2C_REG_RV_CALIBRATION,
					   &mcu_data->rv_calibration, 1);
		if (error) {
			E("%s: CWMCU_i2c_write 1 fails, rc = %d\n",
			  __func__, error);
		} else {
			I("%s: enable RV calibration\n", __func__);
		}
		return count;
	} else if (data_temp == 8) {
		mcu_data->rv_calibration = 0;
		I("%s: Writing, reg_addr = 0x%x, value = 0x%x\n", __func__,
		  CW_I2C_REG_RV_CALIBRATION, mcu_data->rv_calibration);
		error = CWMCU_i2c_write_power(s_mcu_data,
					   CW_I2C_REG_RV_CALIBRATION,
					   &mcu_data->rv_calibration, 1);
		if (error) {
			E("%s: CWMCU_i2c_write 0 fails, rc = %d\n",
			  __func__, error);
		} else {
			I("%s: disnable RV calibration\n", __func__);
		}
		return count;
	} else if (data_temp == 10) {
		mcu_data->rv_calibration = 2;
		I("%s: Use AKM accuracy for Rotation Vector\n", __func__);
		return count;
	}
                               ...
                               const char *buf, size_t count)
{
       ...
       I("%s: Target sensor ID = %d\n", __func__, sensors_id);
       I("%s: cali_data(high, low) = (0x%08X, 0x%08X)\n", __func__,
                                               cali_data_h, cali_data_l);

       switch (sensors_id) {
       case CW_ACCELERATION:
		mcu_data->gs_kvalue = cali_data_h;
		break;
       case CW_GYRO:
		mcu_data->gy_kvalue = cali_data_h;
		break;
       case CW_LIGHT:
		mcu_data->als_kvalue = cali_data_h;
		break;
       case CW_PROXIMITY:
		mcu_data->ps_kheader = cali_data_h;
		mcu_data->ps_kvalue = cali_data_l;
		break;
       default:
		break;
       }
                               ...
                               struct device_attribute *attr, const char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "GS[0x%08X], GYRO[0x%08X], "
				...
				mcu_data->gs_kvalue, mcu_data->gy_kvalue,
				mcu_data->als_kvalue, mcu_data->ps_kheader,
				mcu_data->ps_kvalue);
}
			     ...
			     const char *buf, size_t count)
{
	...
	if (len < sizeof(mcu_rdata)) {
		...
		for (i = 0; i < len; i++) {
			I("read mcu reg_addr = 0x%x, reg[0x%x] = 0x%x\n",
...
#ifdef IS_MTK_PLATFORM
			mt_set_gpio_dir(mcu_data->gpio_reset, 0);
			mt_set_gpio_mode(mcu_data->gpio_reset, 0);
#else
...
#ifdef IS_MTK_PLATFORM
			write_res = i2c_write_dma(mcu_data->client, reg_addr, data, len);
#else
...
static int cwmcu_restore_status(struct cwmcu_data *mcu_data)
{
	...
	rc = CWMCU_i2c_write_power(mcu_data, CW_I2C_REG_WARN_MSG_ENABLE,
			     &reg_value, 1);
	if (rc) {
		E("%s: CWMCU_i2c_write_power(WARN_MSG) fails, rc = %d, i = %d\n",
		  __func__, rc, i);
		return -EIO;
	}
	...
	rc = CWMCU_i2c_write_power(mcu_data, CW_I2C_REG_WATCH_DOG_ENABLE,
			     &reg_value, 1);
	if (rc) {
		E("%s: CWMCU_i2c_write_power(WATCH_DOG) fails, rc = %d\n",
		  __func__, rc);
		return -EIO;
	}
	...
	if (mcu_data->rv_calibration == 1) {
		...
		rc = CWMCU_i2c_write_power(s_mcu_data,
					   ...
					   &mcu_data->rv_calibration, 1);
		if (rc) {
			E("%s: write REG_RV_CALIBRATION fails, rc = %d\n",
			  __func__, rc);
		}
	}
    ...
    uint8_t *data, int length)
{
	...
	if (ret < 0){
		E("Dma I2C Write Error: 0x%04X, %d byte(s), err-code: %d", addr, length, ret);
		mutex_unlock(&dma_i2c_lock);
		return ret;
	}
	mutex_unlock(&dma_i2c_lock);
			 ...
			 u8 reg_addr, void *data, u8 len)
{
	...
	while (!retry_exhausted(mcu_data)) {
#ifdef IS_MTK_PLATFORM
        rc = i2c_read_dma(mcu_data->client, reg_addr, data, len);
        if (rc >= 0) {
#else
...
#ifdef IS_MTK_PLATFORM
		mt_set_gpio_dir(mcu_data->gpio_reset, 1);
        mt_set_gpio_mode(mcu_data->gpio_reset, 0);
        mt_set_gpio_out(mcu_data->gpio_reset, 0);
#else
...
static ssize_t rv_cali_show(struct device *dev, struct device_attribute *attr,
		      char *buf)
{
	struct cwmcu_data *mcu_data = dev_get_drvdata(dev);
	int ret;
	...
	ret = CWMCU_i2c_read_power(mcu_data, CW_I2C_REG_RV_CALI_THD,
				   &data, sizeof(data));
	if (ret < 0) {
		D("%s: Read CW_I2C_REG_RV_CALI_THD fail, ret = %d\n",
		  __func__, ret);
	}

	I("%s: CW_I2C_REG_RV_CALI_THD = %d\n", __func__, data);

	return scnprintf(buf, PAGE_SIZE, "RV Calibration Threshold = %d\n",
			 data);
}

static ssize_t rv_cali_set(struct device *dev, struct device_attribute *attr,
		     const char *buf, size_t count)
{
	struct cwmcu_data *mcu_data = dev_get_drvdata(dev);
	u8 data;
	...
	rc = CWMCU_i2c_write_power(mcu_data, CW_I2C_REG_RV_CALI_THD, &data, 1);
	if (rc) {
		E("%s: write CW_I2C_REG_RV_CALI_THD fails, rc = %d\n",
		  __func__, rc);
	}
		       ...
		       __le64 *data64, u32 *event_count, bool is_wake)
{
	...
	if (data[0] == CW_META_DATA) {
		...
		*handle_time_base = timestamp;
		...
		if ((data[0] == CW_MAGNETIC) || (data[0] == CW_ORIENTATION)) {
		...
		} else if ((mcu_data->rv_calibration == 1)
			   && (data[0] == CW_ROTATIONVECTOR)) {
			ret = CWMCU_i2c_read(mcu_data,
					     CWSTM32_BATCH_MODE_DATA_QUEUE,
					     data, 9);
			(*i)++;
			if ((data[0] == CW_ROTATIONVECTOR_ACCURACY)
			    && (ret >= 0)) {
				bias_event[0] = le16_to_cpup(data16 + 1);
				bias_event[1] = le16_to_cpup(data16 + 2);
				bias_event[2] = le16_to_cpup(data16 + 3);
				...
				bias_event[2] = 0;
				E(
				  "%s: Read RV calibration data fails, "
				  "ret = %d, sensors_id = %d\n",
				  __func__, ret, data[0]);
			}
			...
			D(
			  ...
			  "(%d, %d, %d), bias(x, y,  z) = "
			  "(%d, %d, %d)\n"
			  , *event_count, *i, CW_ROTATIONVECTOR
			  , data_event[0], data_event[1], data_event[2]
			  , bias_event[0], bias_event[1]
			  , bias_event[2]);
			cw_send_event_special(mcu_data, CW_ROTATIONVECTOR,
					      data_event,
					      bias_event,
					      timestamp_event +
					      *handle_time_base);

		} else if ((mcu_data->rv_calibration == 2)
			   && (data[0] == CW_ROTATIONVECTOR)) {
			int rc;
			u8 accuracy;
			u16 bias_event[REPORT_EVENT_COMMON_LEN] = {0};
			...
			if (rc < 0) {
				E(
				  "%s: read ACCURACY_MAG for RV fails, rc = "
				  "%d\n", __func__, rc);
				accuracy = 3;
			}

			if ((accuracy == 1)
			    && (mcu_data->prev_rv_accuracy <= accuracy)) {
				D("Ignore accuracy == 1\n");
				accuracy = 0;
			}

			bias_event[0] = (accuracy << 8) | ((!accuracy) ? 1 : 0);

			if ((accuracy == 0)
			    && (mcu_data->prev_rv_accuracy > accuracy)
			    && time_after(jiffies,
					  mcu_data->popup_compass_dialog_jiffies
					  + COMPASS_DIALOG_DEBOUNCE)) {
				bias_event[1] = 1; 
				mcu_data->popup_compass_dialog_jiffies =
					jiffies;
			} else {
			...
			if (mcu_data->prev_rv_accuracy != accuracy) {
				I(
				  "Batch data: total count = %u, current "
				  "count = %d, event_id = %d, accuracy = %d"
				  ", bias_event[0] = 0x%x\n"
				  , *event_count, *i, CW_ROTATIONVECTOR
				  , accuracy, bias_event[0]);
			}
...
static irqreturn_t cwmcu_irq_handler(int irq, void *handle)
{
    ...
    D("[CWMCU] %s\n", __func__);
    queue_work(mcu_data->mcu_wq, &mcu_data->mcu_irq_work);
    return IRQ_HANDLED;
}

static void cwmcu_irq_work(struct work_struct *work)
{
    struct cwmcu_data *mcu_data = container_of(work,
            struct cwmcu_data, mcu_irq_work);
	s32 ret;
	...
	if (MCU_IN_DLOAD() || MCU_IN_BOOTLOADER()
#ifdef SHUB_DLOAD_SUPPORT
		|| mcu_data->gpio_mcu_status_level == MCU2CPU_STATUS_GPIO_LEVEL_DLOAD
#endif
		) {
...
static int mcu_parse_dt(struct device *dev, struct cwmcu_data *pdata)
{
	...
	if (baro_sensor_offset) {
		...
		E("%s: Barometer-sensor Calibration data offset not found\n",
			__func__);
#ifdef IS_MTK_PLATFORM
    prop = of_find_property(dt, "mcu,mcu_status-gpio", NULL);
    if (prop) {
        of_property_read_u32(dt, "mcu,mcu_status-gpio", &buf);
        pdata->gpio_mcu_status = buf;
        D("%s: gpio_mcu_status = %d\n", __func__, pdata->gpio_mcu_status);
    } else
        I("%s: gpio_mcu_status not found", __func__);


    if (!gpio_is_valid(pdata->gpio_mcu_status))
        E("DT:gpio_mcu_status value is not valid\n");
    else
        I("DT:gpio_mcu_status=%d\n", pdata->gpio_mcu_status);


    prop = of_find_property(dt, "mcu,Cpu_wake_mcu-gpio", NULL);
    if (prop) {
        of_property_read_u32(dt, "mcu,Cpu_wake_mcu-gpio", &buf);
        pdata->gpio_wake_mcu = buf;
        D("%s: gpio_wake_mcu = %d\n", __func__, pdata->gpio_wake_mcu);
    } else
        I("%s: gpio_wake_mcu not found", __func__);
    ...
    prop = of_find_property(dt, "mcu,intr-gpio", NULL);
    if (prop) {
        of_property_read_u32(dt, "mcu,intr-gpio", &buf);
        pdata->gpio_mcu_irq = buf;
        D("%s: gpio_mcu_irq = %d\n", __func__, pdata->gpio_mcu_irq);
    } else
        I("%s: gpio_mcu_irq not found", __func__);
    if (!gpio_is_valid(pdata->gpio_mcu_irq))
        E("DT:gpio_mcu_irq is not valid\n");
    else
        I("DT:gpio_mcu_irq=%d\n", pdata->gpio_mcu_irq);


    prop = of_find_property(dt, "mcu,Reset-gpio", NULL);
    if (prop) {
        of_property_read_u32(dt, "mcu,Reset-gpio", &buf);
        pdata->gpio_reset = buf;
        D("%s: gpio_reset = %d\n", __func__, pdata->gpio_reset);
    } else
        I("%s: gpio_reset not found", __func__);
    ...
    prop = of_find_property(dt, "mcu,Chip_mode-gpio", NULL);
    if (prop) {
        of_property_read_u32(dt, "mcu,Chip_mode-gpio", &buf);
        pdata->gpio_chip_mode = buf;
        D("%s: mcu,gpio_chip_mode = %d\n", __func__, pdata->gpio_chip_mode);
    } else
        I("%s: mcu,gpio_chip_mode not found", __func__);
    if (!gpio_is_valid(pdata->gpio_chip_mode))
        E("DT:gpio_chip_mode is not valid\n");
    else
        I("DT:gpio_chip_mode=%d\n", pdata->gpio_chip_mode);

    const char *temp_string;
    prop = of_find_property(dt, "mcu,SR_1V8", NULL);
    if (prop) {
        of_property_read_string(dt, "mcu,SR_1V8", &temp_string);
        I("%s: mcu,SR_1V8: %s", __func__, temp_string);
        if(!strcmp(temp_string, "MT6331_POWER_LDO_VCAM_AF")) {
            I("%s: turn on MT6331_POWER_LDO_VCAM_AF", __func__);
            hwPowerOn(MT6331_POWER_LDO_VCAM_AF, VOL_1800, "V_SRIO_1V8");
        } else {
            I("%s: mcu,SR_1V8 is not set correctly", __func__);
        }
    } else
        I("%s: mcu,SR_1V8 not found", __func__);
	...
	if (prop) {
        ...
        prop = of_find_property(dt, "mcu,rv_calibration", NULL);
        if (prop) {
                of_property_read_u32(dt, "mcu,rv_calibration", &buf);
                pdata->rv_calibration = buf;
                I("%s: rv_calibration = 0x%x", __func__, pdata->rv_calibration);
        } else {
                pdata->rv_calibration = 0;
                I("%s: rv_calibration not found", __func__);
        }
...
static int create_sysfs_interfaces(struct cwmcu_data *mcu_data)
{
	...
	mcu_data->bma250_class = class_create(THIS_MODULE, "htc_g_sensor");
...
static int mcu_i2c_rx(struct i2c_client *client, u8 *buf, int len)
{
    int ret;
    int i;
...
#ifdef IS_MTK_PLATFORM
        ret = i2c_master_recv(client, (char *)gpDMABuf_pa, (int)len);
...
static int mcu_i2c_tx(struct i2c_client *client, u8 *buf, int len)
{
    int ret;
    int i;
    ...
    for (i = 0; i < RETRY_TIMES; i++) {
        D("%s(%d) len:%d\n", __func__, __LINE__, len);
#ifdef IS_MTK_PLATFORM
        ret = i2c_master_send(client, (char *)gpDMABuf_pa, (int)len);
#else
...
#define HTC_PARAM_SIZE                      (0x000000FD)
...
#define MCU_FW_UPDATE_I2C_LEN_LIMIT 253
    ...
    case SHUB_FW_IOCTL_PRE_FLASH:
        {
			...
			if(g_mhl_wa){
#ifdef IS_MTK_PLATFORM
				mt_set_gpio_dir(GPIO_MHL_RST_B_PIN, 1);
				mt_set_gpio_mode(GPIO_MHL_RST_B_PIN, 0);
				mt_set_gpio_out(GPIO_MHL_RST_B_PIN, 0);
#endif
				I("%s(%d): After enter bootloader, get GPIO_MHL_RST_B_PIN value = %d\n", __func__, __LINE__, gpio_get_value(GPIO_MHL_RST_B_PIN));
			}
#endif
    ...
    case SHUB_FW_IOCTL_POST_FLASH:
        {
			...
			if(g_mhl_wa){
				...
				mt_set_gpio_dir(GPIO_MHL_RST_B_PIN, 1);
				mt_set_gpio_mode(GPIO_MHL_RST_B_PIN, 0);
				mt_set_gpio_out(GPIO_MHL_RST_B_PIN, 1);
#endif
				I("%s(%d): After leave bootloader, get GPIO_MHL_RST_B_PIN value = %d\n", __func__, __LINE__, gpio_get_value(GPIO_MHL_RST_B_PIN));
			}
#endif
                                  ...
                                  const struct i2c_device_id *id)
{
...
#ifdef IS_MTK_PLATFORM
	client->timing = MAX_FS_MODE_SPEED;
#endif
    ...
    prop = of_find_property(dt, "mcu,MHL_WA", NULL);
    if (prop) {
		I("%s: mcu,MHL_WA found", __func__);
		g_mhl_wa = true;
    } else {
		I("%s: mcu,MHL_WA not found", __func__);
		g_mhl_wa = false;
    }
...
#ifdef IS_MTK_PLATFORM
static int cwmcu_irq_registration(unsigned int irq, irq_handler_t handler, unsigned long flags, const char *name, void *dev)
{
  struct device_node *node = NULL;
  int ret = 0; 
  u32 ints[2] = {0,0};

  node = of_find_compatible_node(NULL, NULL, "mediatek, HTC_SENSOR_HUB-eint");
  if(node){
    ...
    ret = request_irq(irq, handler, flags, "HTC_SENSOR_HUB-eint", dev);
    if(ret > 0){
      ret = -1;
      E("sensor hub request_irq IRQ LINE NOT AVAILABLE!.\n");
    }
  }else{
    E("sensor hub request_irq can not find eint device node!.\n");
    ret = -1;
  }    
  I("[%s]irq:%d, debounce:%d-%d: \n", __FUNCTION__, irq, ints[0], ints[1]);
  return ret; 
}
#endif
				       ...
				       const struct i2c_device_id *id)
{
...
#ifdef IS_MTK_PLATFORM
    gpio_set_value(GPIO53, 1);
    hwPowerOn(MT6331_POWER_LDO_VIO28, VOL_2800, "V_SR_2V85");
...
#ifdef IS_MTK_PLATFORM
        mt_set_gpio_dir(mcu_data->gpio_mcu_status, 0);
        mt_set_gpio_mode(mcu_data->gpio_mcu_status, 0);
#else
...
#ifdef IS_MTK_PLATFORM
		mt_set_gpio_dir(mcu_data->gpio_wake_mcu, 1);
		mt_set_gpio_mode(mcu_data->gpio_wake_mcu, 0);
		mt_set_gpio_out(mcu_data->gpio_wake_mcu, 1);
#else
	...
	if (gpio_is_valid(mcu_data->gpio_chip_mode)) {
		...
		mt_set_gpio_dir(mcu_data->gpio_chip_mode, 1);
		mt_set_gpio_mode(mcu_data->gpio_chip_mode, 0);
		mt_set_gpio_out(mcu_data->gpio_chip_mode, 0);
#else
...
#ifdef IS_MTK_PLATFORM
		mt_set_gpio_dir(mcu_data->gpio_mcu_irq, 0);
		mt_set_gpio_mode(mcu_data->gpio_mcu_irq, 0);
#else
    ...
    mt_set_gpio_dir(mcu_data->gpio_reset, 1);
    mt_set_gpio_mode(mcu_data->gpio_reset, 0);
    mt_set_gpio_out(mcu_data->gpio_reset, 1);

    mt_set_gpio_dir(mcu_data->gpio_wake_mcu, 1);
    mt_set_gpio_mode(mcu_data->gpio_wake_mcu, 0);
    mt_set_gpio_out(mcu_data->gpio_wake_mcu, 0);
    ...
    INIT_WORK(&mcu_data->mcu_irq_work, cwmcu_irq_work);
...
#ifdef IS_MTK_PLATFORM
    mt_set_gpio_dir(mcu_data->gpio_mcu_irq, GPIO_DIR_IN);
    mt_set_gpio_mode(mcu_data->gpio_mcu_irq, 0);
    mt_set_gpio_pull_enable(mcu_data->gpio_mcu_irq, TRUE);
    mt_set_gpio_pull_select(mcu_data->gpio_mcu_irq, GPIO_PULL_DOWN);

    error = cwmcu_irq_registration(client->irq, cwmcu_irq_handler,
            IRQF_TRIGGER_RISING, client->name, mcu_data);
    if (error != 0) { 
            E("Failed to setup IRQ handler\n");
            error = -EIO;
            goto err_device_init_irq;
    }    
    I("(INIT): IRQ handler OK client->irq:%d\n", client->irq);
	...
	mcu_data->early_suspend.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING - 1;
	mcu_data->early_suspend.suspend = early_suspend;
	mcu_data->early_suspend.resume = late_resume;
	register_early_suspend(&mcu_data->early_suspend);
	mcu_data->early_suspend_registered = 1;
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
		if (mcu_data->early_suspend_registered)
			unregister_early_suspend(&mcu_data->early_suspend);
#endif
